...
class VisualIDExportHandler(ImageExportHandler):
    _files = [
    ...
    def __init__(self, sub_parser):
        ...
        self.path = sub_parser.add_parser(
            'visualid',
            help='Export based on IDs in ItemVisualIdentity.dat',
            formatter_class=argparse.RawTextHelpFormatter,
        )
        
        # Add and user image args?

        self.add_id_arguments(self.path)
    ...
    def handle(self, args):
        ...
        item_vis_ids = self.rr['ItemVisualIdentity.dat']
        file_paths = []
        for item_vis_id in item_vis_ids:
            if item_vis_id['Id'] in target_ids:
                file_paths.append(item_vis_id['DDSFile'])
